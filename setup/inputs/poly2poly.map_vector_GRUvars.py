...
def findModeVal(wgt, var_in, varname):    
    ...
    for h in range(start_poly,(end_poly+1)):        
        ...
        # mode value has highest (max) weight of contributing cells
        index = [i for i, x in enumerate(currWgtVal) if x == max(currWgtVal)]
        index = index[0]    # if there are matching max weights, take 1st one (from list)

        # check if inPoly at index is valid
        if currInPolyIndex[index] >= 0 and dataVals[currInPolyIndex[index]] <= 1.e+19:

            print(" found %s mode data %f for in_poly_index %d" %
                  (varname, dataVals[currInPolyIndex[index]], currInPolyIndex[index]))

            # store weighted average results
	    if 'wgtedVals' in locals():
                wgtedVals=np.append(wgtedVals, dataVals[currInPolyIndex[index]])
                activeOutPolyIDs = np.append(activeOutPolyIDs, outPolyIDs[h])
            else:
  	        # first time, create the start fof the list to be summed
                wgtedVals = dataVals[currInPolyIndex[index]]
                activeOutPolyIDs = outPolyIDs[h]                
        ...
        # else none of the contributing cells are present and avg can't be formed
            ...
            print("WARNING: mode cell (input index %d)" % currInPolyIndex[index] +
                  " in hru %d is void or missing: omitting HRU from output" % (h))
